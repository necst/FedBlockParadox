...
from .block import ModelBlock, ModelBlockInvolvedTrainerFields, GenesisBlock

class CommitteeBasedNode(GenericNode):
	...
	def __init__(self, logger: diagnostic.Diagnostic, node_id: int, host: str, port: int, dataset_quanta_paths: list, archive_host: str, archive_port: int, lazy_loading: bool, test_set_path: str, val_set_path: str, malicious_training_semaphore: BoundedSemaphoreClass, honest_training_semaphore: BoundedSemaphoreClass, validation_semaphore: BoundedSemaphoreClass, fit_epochs: int = 3, batch_size: int = 32, already_available_peers: dict = {}, allowed_to_write_redudant_log_messages: bool = False, test_set_validation_of_global_model_after_each_round: bool = False, genesis_block = None, is_active_trainer_in_first_round: (bool | None) = None, store_weights_directly_in_archive_tmp_dir: bool = False, archive_tmp_dir: (str | None) = None, prepare_node_for_first_round: bool = True):
		...
		if type(prepare_node_for_first_round) != bool:
			raise TypeError("CommitteeBasedNode constructor")
		...
		#
		# CommitteeBasedNode specific initializations
		#
	...
	def _elect_new_validators(self, updater_id_aggregated_validation_scores: dict):
		...
		"""
		try:
			if type(updater_id_aggregated_validation_scores) != dict:
				raise TypeError("Node _elect_new_validators method")

			elif len(updater_id_aggregated_validation_scores) < self._num_of_validators:
				raise ValueError(f"Node _elect_new_validators method. Not enough scores to elect the new validators. Number of validators: {self._num_of_validators}. Available scores: {updater_id_aggregated_validation_scores}")
			
			with self._validators_list_lock:
				self._is_validator = False
				
				# Elect the new validators
				# With this method if there are more users with the same score, the users are randomly selected
				new_validators = list()
				max_score = None
				max_score_users = list()
				used_scores = list()
				seed = self.aggregation_round()

				while len(new_validators) < self._num_of_validators:
					for user, score in updater_id_aggregated_validation_scores.items():
					...
					if len(max_score_users) == 0 or max_score is None:
						raise ValueError(f"No score available to elect the new validators. Max score: {max_score}. Max score users: {max_score_users}")
					
					if len(max_score_users) > (self._num_of_validators - len(new_validators)):
						new_validators.extend(self._get_random_elements_from_list(max_score_users, self._num_of_validators - len(new_validators), seed))
					
					else:
						new_validators.extend(max_score_users)

					seed += len(max_score_users)
					used_scores.append(max_score)
					max_score = None
					max_score_users = list()
				
				if len(new_validators) != self._num_of_validators:
					raise ValueError(f"Invalid number of validators. Expected number of validators: {self._num_of_validators}. Number of validators: {len(new_validators)}. List of validators: {new_validators}")

				if self._allowed_to_write_redudant_log_messages:
					self._logger.record(msg = f"New validators elected. Validator ids: {new_validators}. Round: {self.aggregation_round()}", logLevel= diagnostic.DEBUG, identifier= self._str_identifier)
				
				self._validators_list = new_validators
	...
	def _prepare_node_for_next_round(self, aggregated_model_block_hash: str, aggregated_model_weights: (list | None) = None, aggregated_model_optimizer_variables: (dict | None) = None, available_nodes_for_next_round: list = None, updater_ids_update_names_aggregated_validation_scores: list = None):
		...
		try:
			...
			if stop_training is False:
				...
				# Elect the new validators based on the aggregated validation scores of the updates of the previous round
				updater_id_aggregated_validation_scores = dict()
				for element in updater_ids_update_names_aggregated_validation_scores:
					updater_id_aggregated_validation_scores[element[ModelBlockInvolvedTrainerFields.NODE_ID]] = element[ModelBlockInvolvedTrainerFields.UPDATE_SCORE]
					list_of_honest_updates_aggregated_validation_scores.append(element[ModelBlockInvolvedTrainerFields.UPDATE_SCORE])
				
				self._elect_new_validators(updater_id_aggregated_validation_scores)
	...
	def _create_and_upload_aggregated_model_block(self, update_names_and_aggregated_scores: dict):
		...
		try:
			...
			for update_name in update_names_and_aggregated_scores:
				if self._allowed_to_store_weights_directly_in_archive_tmp_dir:
			...
			# Define the list of available nodes for the next round
