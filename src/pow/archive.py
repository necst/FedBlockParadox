...
class PowBasedArchive(GenericArchive):
	...
	def _handle_upload_blockchain_aggregated_model_request(self, msg: dict):
		...
		nodes_computing_power_factor = msg[am.UploadBlockchainAggregatedModelRequestBody.NODES_COMPUTING_POWER_FACTOR]
		...
		try:
			with self._current_round_number_lock:
				with self._blockchain_lock:
					...
					model = ModelBlock(previous_hash, timestamp, model_name, round, involved_trainers, list_of_current_validators, available_nodes, nodes_computing_power_factor)
