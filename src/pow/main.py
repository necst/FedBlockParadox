import os, json, time, sys, gc, setproctitle
...
from ..shared.enums import common as cm, peer_generic as pg, validation_algos as va
...
from .enums.common import AdditionalPowSpecificConfigParams, NodesComputingPowerConfigElemFields, PowBasedAttackerSpecificConfigParams, PowBasedNodeSpecificConfigParams
from .constants import DEFAULT_POW_SPECIFIC_CONFIG
from .block import GenesisBlock, NodesComputingPowerElemFields
from .archive import PowBasedArchive
from .node import PowBasedNode
from .attacker_nodes import PowBasedAdditiveNoiseByzantineNode, PowBasedRandomLabelByzantineNode, PowBasedRandomNoiseByzantineNode, PowBasedTargetedPoisoningNode, PowBasedLabelFlippingNode
...
def validate_pow_consensus_specific_config(stored_config: dict, run_time_only_params: dict) -> None:
	'''
	Define the pow specific running configuration from the stored configuration.
	'''

	try:
		if type(stored_config) != dict or type(run_time_only_params) != dict:
			raise TypeError("validate_pow_consensus_specific_config")
		elif cm.ConfigParams.CONSENSUS_PARAMS not in stored_config:
			raise KeyError("validate_pow_consensus_specific_config. Consensus params not in stored configuration")
		elif type(stored_config[cm.ConfigParams.CONSENSUS_PARAMS]) != dict:
			raise TypeError("validate_pow_consensus_specific_config. Consensus params is not a dictionary")
		elif cm.ConfigGenericConsensusAlgorithmParams.TYPE not in stored_config[cm.ConfigParams.CONSENSUS_PARAMS] or stored_config[cm.ConfigParams.CONSENSUS_PARAMS][cm.ConfigGenericConsensusAlgorithmParams.TYPE] != cm.ConsensusAlgorithmType.POW:
			raise ValueError("validate_pow_consensus_specific_config. Consensus algorithm type is not pow specific")
		elif any(key not in run_time_only_params for key in cm.RunTimeOnlyParams.list()):
			raise KeyError("validate_pow_consensus_specific_config. Run time only params missing key")
		...
		for key in AdditionalPowSpecificConfigParams.list():
			if key not in consensus_specific_config:
				raise KeyError(f"validate_pow_consensus_specific_config. {key} not in stored configuration")
		...
		if is_support_machine_config is True:
			if consensus_specific_config[AdditionalPowSpecificConfigParams.LIST_OF_ACTIVE_TRAINERS_IN_THE_FIRST_ROUND] is None:
				raise ValueError("validate_pow_consensus_specific_config. List of active trainers in the first round is None but it is required for the support machine")

		if consensus_specific_config[AdditionalPowSpecificConfigParams.LIST_OF_ACTIVE_TRAINERS_IN_THE_FIRST_ROUND] is not None:
			if type(consensus_specific_config[AdditionalPowSpecificConfigParams.LIST_OF_ACTIVE_TRAINERS_IN_THE_FIRST_ROUND]) != list:
				raise TypeError("validate_pow_consensus_specific_config. List of active trainers in the first round is not a list")
			elif any(type(elem) != int or elem < 0 for elem in consensus_specific_config[AdditionalPowSpecificConfigParams.LIST_OF_ACTIVE_TRAINERS_IN_THE_FIRST_ROUND]):
				raise TypeError("validate_pow_consensus_specific_config. List of active trainers in the first round contains an element that is not an integer or is less than zero")
			
			for node_id in consensus_specific_config[AdditionalPowSpecificConfigParams.LIST_OF_ACTIVE_TRAINERS_IN_THE_FIRST_ROUND]:
				if any(node_id == node[PowBasedNodeSpecificConfigParams.NODE_ID] for node in run_time_only_params[cm.RunTimeOnlyParams.NODES]) is False:
					raise ValueError("validate_pow_consensus_specific_config. List of active trainers in the first round contains a node id that is not part of the simulation")

		# The 'nodes_computing_power' field can be defined for both the main simulation and the support machine
		node_ids_considered_in_computing_power = []
		
		for nodes_computing_power_elem in consensus_specific_config[AdditionalPowSpecificConfigParams.NODES_COMPUTING_POWER]:
			for key in NodesComputingPowerConfigElemFields.list():
				if key not in nodes_computing_power_elem:
					raise KeyError(f"validate_pow_consensus_specific_config. {key} not in nodes computing power element")
			
			if type(nodes_computing_power_elem[NodesComputingPowerConfigElemFields.ALIAS]) != str or type(nodes_computing_power_elem[NodesComputingPowerConfigElemFields.NODE_IDS]) != list or type(nodes_computing_power_elem[NodesComputingPowerConfigElemFields.COMPUTING_POWER_FACTOR]) not in [float, int]:
				raise TypeError("validate_pow_consensus_specific_config. Invalid nodes computing power element")
			elif nodes_computing_power_elem[NodesComputingPowerConfigElemFields.COMPUTING_POWER_FACTOR] <= 0.0 or nodes_computing_power_elem[NodesComputingPowerConfigElemFields.COMPUTING_POWER_FACTOR] > 1.0:
				raise ValueError("validate_pow_consensus_specific_config. Invalid computing power factor. It must be greater than 0 and less or equal than 1")

			node_ids_considered_in_computing_power += nodes_computing_power_elem[NodesComputingPowerConfigElemFields.NODE_IDS]

		for node in run_time_only_params[cm.RunTimeOnlyParams.NODES]:
			if node[cm.RunTimeNodeParams.NODE_ID] not in node_ids_considered_in_computing_power:
				raise ValueError(f"validate_pow_consensus_specific_config. Node id {node[cm.RunTimeNodeParams.NODE_ID]} is not considered in the nodes computing power")

		if is_support_machine_config is False:
			# Percentage of trainers active in each round
			if consensus_specific_config[AdditionalPowSpecificConfigParams.PERC_OF_TRAINERS_ACTIVE_IN_EACH_ROUND] is None:
				raise ValueError("validate_pow_consensus_specific_config. Percentage of trainers active in each round is None")
			elif type(consensus_specific_config[AdditionalPowSpecificConfigParams.PERC_OF_TRAINERS_ACTIVE_IN_EACH_ROUND]) not in [float, int]:
				raise TypeError("validate_pow_consensus_specific_config. Percentage of trainers active in each round is not a float or int")
			elif consensus_specific_config[AdditionalPowSpecificConfigParams.PERC_OF_TRAINERS_ACTIVE_IN_EACH_ROUND] < 0.0 or consensus_specific_config[AdditionalPowSpecificConfigParams.PERC_OF_TRAINERS_ACTIVE_IN_EACH_ROUND] > 1.0:
				raise ValueError("validate_pow_consensus_specific_config. Percentage of trainers active in each round is not between 0 and 1")
			...
			num_of_trainers_in_each_round = round(consensus_specific_config[AdditionalPowSpecificConfigParams.PERC_OF_TRAINERS_ACTIVE_IN_EACH_ROUND] * len(run_time_only_params[cm.RunTimeOnlyParams.NODES]))
			...
			if stored_config[cm.ConfigParams.VALIDATION_PARAMS][cm.ConfigGenericConsensusAlgorithmParams.TYPE] == cm.ValidationAlgorithmType.LOCAL_DATASET_VALIDATION:
				if num_of_trainers_in_each_round < stored_config[cm.ConfigParams.VALIDATION_PARAMS][va.LocalDatasetUsedForValidationParams.MINIMUM_NUM_OF_UPDATES_BETWEEN_AGGREGATIONS]:
					raise ValueError("validate_pow_consensus_specific_config. Number of trainers in each round is less than the minimum number of updates between aggregations. Consequently, it is not possible to complete any round")
			...
			# Krum validation
				...
				if num_of_trainers_in_each_round < stored_config[cm.ConfigParams.VALIDATION_PARAMS][va.KrumValidationParams.MINIMUM_NUM_OF_UPDATES_NEEDED_TO_START_VALIDATION]:
					raise ValueError("validate_pow_consensus_specific_config. Number of trainers in each round is less than the minimum number of updates needed to start the validation. Consequently, it is not possible to complete any round")
			...
			# Pass gradients validation
				...
				if num_of_trainers_in_each_round < stored_config[cm.ConfigParams.VALIDATION_PARAMS][va.PassGradientsValidationParams.MINIMUM_NUM_OF_UPDATES_NEEDED_TO_START_VALIDATION]:
					raise ValueError("validate_pow_consensus_specific_config. Number of trainers in each round is less than the number of updates between aggregations. Consequently, it is not possible to complete any round")
			
			else:
				raise ValueError("validate_pow_consensus_specific_config. Invalid validation algorithm")
	
	except Exception as e:
		raise Exception(f"validate_pow_consensus_specific_config. Exception: {e}") from e
...
def start_nodes(process_index: int, logger_path: str, logger_level: int, kill_process_event: EventClass, nodes_specific_params: dict, quanta_paths: list, perc_of_iid_quanta: float, archive_host: str, archive_port: int, fit_epochs: int, batch_size: int, initial_peers_dict: dict, genesis_block, need_to_join_an_existing_network: bool, lazy_loading: bool, test_set_path: str, val_set_path: str, test_set_validation_of_global_model_after_each_round: bool, malicious_training_semaphore: BoundedSemaphoreClass, honest_training_semaphore: BoundedSemaphoreClass, validation_semaphore: BoundedSemaphoreClass, entry_point_nodes: list = [], store_weights_directly_in_archive_tmp_dir: bool = False, archive_tmp_dir: (str | None) = None) -> None:
	...
	try:
		...
		for node_id in nodes_specific_params:
			...
			elif any(key not in nodes_specific_params[node_id] for key in PowBasedNodeSpecificConfigParams.list()):
				raise KeyError("start_nodes method. Invalid node config")

			if type(nodes_specific_params[node_id][PowBasedNodeSpecificConfigParams.NODE_ID]) != int or type(nodes_specific_params[node_id][PowBasedNodeSpecificConfigParams.HOST]) != str or type(nodes_specific_params[node_id][PowBasedNodeSpecificConfigParams.PORT]) != int or type(nodes_specific_params[node_id][PowBasedNodeSpecificConfigParams.ALLOWED_TO_PRODUCE_DEBUG_LOG_MESSAGES]) != bool or (type(nodes_specific_params[node_id][PowBasedNodeSpecificConfigParams.ACTIVE_TRAINER_IN_FIRST_ROUND]) != bool and nodes_specific_params[node_id][PowBasedNodeSpecificConfigParams.ACTIVE_TRAINER_IN_FIRST_ROUND] is not None) or isinstance(nodes_specific_params[node_id][PowBasedNodeSpecificConfigParams.PAUSE_RESUME_EVENT], EventClass) is False:
				raise TypeError("start_nodes method. Invalid node config param")
			elif nodes_specific_params[node_id][PowBasedNodeSpecificConfigParams.PORT] < 0 or nodes_specific_params[node_id][PowBasedNodeSpecificConfigParams.PORT] > 65535 or nodes_specific_params[node_id][PowBasedNodeSpecificConfigParams.NUM_OF_IID_QUANTA_TO_USE] < 0 or nodes_specific_params[node_id][PowBasedNodeSpecificConfigParams.NUM_OF_QUANTA_TO_USE] < 0 or nodes_specific_params[node_id][PowBasedNodeSpecificConfigParams.PAUSE_RESUME_EVENT].is_set():
			...
			if not nodes_specific_params[node_id][PowBasedNodeSpecificConfigParams.MALICIOUS]:
				nodes_threads.append(
					PowBasedNode(
						logger,
						nodes_specific_params[node_id][PowBasedNodeSpecificConfigParams.NODE_ID],
						nodes_specific_params[node_id][PowBasedNodeSpecificConfigParams.HOST],
						nodes_specific_params[node_id][PowBasedNodeSpecificConfigParams.PORT],
						selected_quanta_paths,
						archive_host,
						archive_port,
						lazy_loading,
						nodes_specific_params[node_id][PowBasedNodeSpecificConfigParams.COMPUTING_POWER_FACTOR],
						test_set_path,
						val_set_path,
						...
						batch_size,
						initial_peers_dict,
						nodes_specific_params[node_id][PowBasedNodeSpecificConfigParams.ALLOWED_TO_PRODUCE_DEBUG_LOG_MESSAGES],
						test_set_validation_of_global_model_after_each_round,
						genesis_block,
						nodes_specific_params[node_id][PowBasedNodeSpecificConfigParams.ACTIVE_TRAINER_IN_FIRST_ROUND],
				...
				if nodes_specific_params[node_id][PowBasedAttackerSpecificConfigParams.TYPE] == cm.MaliciousNodeBehaviourType.LABEL_FLIPPING:
					nodes_threads.append(
						PowBasedLabelFlippingNode(
							logger,
							nodes_specific_params[node_id][PowBasedAttackerSpecificConfigParams.NODE_ID],
							nodes_specific_params[node_id][PowBasedAttackerSpecificConfigParams.HOST],
							nodes_specific_params[node_id][PowBasedAttackerSpecificConfigParams.PORT],
							selected_quanta_paths,
							archive_host,
							archive_port,
							lazy_loading,
							nodes_specific_params[node_id][PowBasedAttackerSpecificConfigParams.COMPUTING_POWER_FACTOR],
							test_set_path,
							val_set_path,
							malicious_training_semaphore,
							honest_training_semaphore,
							...
							batch_size,
							initial_peers_dict,
							nodes_specific_params[node_id][PowBasedAttackerSpecificConfigParams.ALLOWED_TO_PRODUCE_DEBUG_LOG_MESSAGES],
							test_set_validation_of_global_model_after_each_round,
							genesis_block,
							nodes_specific_params[node_id][PowBasedAttackerSpecificConfigParams.ACTIVE_TRAINER_IN_FIRST_ROUND],
							store_weights_directly_in_archive_tmp_dir,
							archive_tmp_dir,
							nodes_specific_params[node_id][PowBasedAttackerSpecificConfigParams.STARTING_ROUND_FOR_MALICIOUS_BEHAVIOUR],
							nodes_specific_params[node_id][PowBasedAttackerSpecificConfigParams.SELECTED_CLASSES],
							nodes_specific_params[node_id][PowBasedAttackerSpecificConfigParams.TARGET_CLASSES],
							nodes_specific_params[node_id][PowBasedAttackerSpecificConfigParams.NUM_OF_SAMPLES]
						)
					)
				
				elif nodes_specific_params[node_id][PowBasedAttackerSpecificConfigParams.TYPE] == cm.MaliciousNodeBehaviourType.TARGETED_POISONING:
					nodes_threads.append(
						PowBasedTargetedPoisoningNode(
							logger,
							nodes_specific_params[node_id][PowBasedAttackerSpecificConfigParams.NODE_ID],
							nodes_specific_params[node_id][PowBasedAttackerSpecificConfigParams.HOST],
							nodes_specific_params[node_id][PowBasedAttackerSpecificConfigParams.PORT],
							...
							archive_tmp_dir,
							nodes_specific_params[node_id][PowBasedAttackerSpecificConfigParams.STARTING_ROUND_FOR_MALICIOUS_BEHAVIOUR],
							nodes_specific_params[node_id][PowBasedAttackerSpecificConfigParams.TARGET_CLASS],
							nodes_specific_params[node_id][PowBasedAttackerSpecificConfigParams.SQUARE_SIZE],
							nodes_specific_params[node_id][PowBasedAttackerSpecificConfigParams.NUM_OF_SAMPLES]
						)
					)
				
				elif nodes_specific_params[node_id][PowBasedAttackerSpecificConfigParams.TYPE] == cm.MaliciousNodeBehaviourType.RANDOM_LABEL:
					nodes_threads.append(
						PowBasedRandomLabelByzantineNode(
							logger,
							nodes_specific_params[node_id][PowBasedAttackerSpecificConfigParams.NODE_ID],
							nodes_specific_params[node_id][PowBasedAttackerSpecificConfigParams.HOST],
							nodes_specific_params[node_id][PowBasedAttackerSpecificConfigParams.PORT],
				...
				elif nodes_specific_params[node_id][PowBasedAttackerSpecificConfigParams.TYPE] == cm.MaliciousNodeBehaviourType.ADDITIVE_NOISE:
					nodes_threads.append(
						PowBasedAdditiveNoiseByzantineNode(
							logger,
							nodes_specific_params[node_id][PowBasedAttackerSpecificConfigParams.NODE_ID],
							nodes_specific_params[node_id][PowBasedAttackerSpecificConfigParams.HOST],
							nodes_specific_params[node_id][PowBasedAttackerSpecificConfigParams.PORT],
							...
							archive_tmp_dir,
							nodes_specific_params[node_id][PowBasedAttackerSpecificConfigParams.STARTING_ROUND_FOR_MALICIOUS_BEHAVIOUR],
							nodes_specific_params[node_id][PowBasedAttackerSpecificConfigParams.SIGMA],
							nodes_specific_params[node_id][PowBasedAttackerSpecificConfigParams.NUM_OF_SAMPLES]
						)
					)
				
				elif nodes_specific_params[node_id][PowBasedAttackerSpecificConfigParams.TYPE] == cm.MaliciousNodeBehaviourType.RANDOM_NOISE:
					nodes_threads.append(
						PowBasedRandomNoiseByzantineNode(
							logger,
							nodes_specific_params[node_id][PowBasedAttackerSpecificConfigParams.NODE_ID],
							nodes_specific_params[node_id][PowBasedAttackerSpecificConfigParams.HOST],
							nodes_specific_params[node_id][PowBasedAttackerSpecificConfigParams.PORT],
					...
					raise ValueError(f"start_nodes method. Invalid malicious node behaviour. Behaviour: {nodes_specific_params[node_id][PowBasedAttackerSpecificConfigParams.TYPE]}")
					
			# The node has already been initialized
			nodes_specific_params[node_id][PowBasedNodeSpecificConfigParams.PAUSE_RESUME_EVENT].set()
			print(f"Node {node_id} initialized")

		# Wait for the main process to start the node
		while True:
			if all(nodes_specific_params[node_id][PowBasedNodeSpecificConfigParams.PAUSE_RESUME_EVENT].is_set() is False for node_id in nodes_specific_params) or kill_process_event.is_set():
				break
		...
		if need_to_join_an_existing_network:
			...
			# Pow-specific nodes in a support simulation must broadcast their computing power factor to the other nodes
			for node in nodes_threads:
				node.broadcast_and_apply_node_computing_power_factor()
...
def start_pow_simulation_main(logger: diagnostic.Diagnostic, quanta_paths: list, testset_path: str, valset_path: str, config_file_path: str, preloaded_config: dict, run_time_only_params: dict) -> None:
	'''Start the pow-based simulation'''
	...
	if isinstance(logger, diagnostic.Diagnostic) is False or type(quanta_paths) != list or type(testset_path) != str or type(valset_path) != str or type(config_file_path) != str or type(preloaded_config) != dict or type(run_time_only_params) != dict:
		raise TypeError("start_pow_simulation_main")
		...
		raise FileNotFoundError("start_pow_simulation_main")
	elif any(key not in preloaded_config for key in cm.ConfigParams.list()):
		raise KeyError("start_pow_simulation_main. cm.ConfigParams missing key")
	elif any(key not in run_time_only_params for key in cm.RunTimeOnlyParams.list()):
		raise KeyError("start_pow_simulation_main. Run time only params missing key")

	is_support_simulation = not preloaded_config[cm.ConfigParams.IS_MAIN_SIMULATION]
	
	# Load the configuration file
	preloaded_config[cm.ConfigParams.CONSENSUS_PARAMS] = {**DEFAULT_POW_SPECIFIC_CONFIG, **preloaded_config[cm.ConfigParams.CONSENSUS_PARAMS]}

	validate_pow_consensus_specific_config(preloaded_config, run_time_only_params)
	...
	nodes_computing_power = list()

	for elem in preloaded_config[cm.ConfigParams.CONSENSUS_PARAMS][AdditionalPowSpecificConfigParams.NODES_COMPUTING_POWER]:
		nodes_computing_power.append({NodesComputingPowerElemFields.NODE_IDS: elem[NodesComputingPowerConfigElemFields.NODE_IDS], NodesComputingPowerElemFields.COMPUTING_POWER_FACTOR: elem[NodesComputingPowerConfigElemFields.COMPUTING_POWER_FACTOR]})
	...
	if is_support_simulation is False:
		genesis_block = processes_shared_variables_manager.GenesisBlock(
			...
			# Consensus params		
			preloaded_config[cm.ConfigParams.CONSENSUS_PARAMS][AdditionalPowSpecificConfigParams.PERC_OF_TRAINERS_ACTIVE_IN_EACH_ROUND],
			# Validation params
		...
		if preloaded_config[cm.ConfigParams.ARCHIVE_PARAMS][cm.ConfigArchiveParams.ARCHIVE_MUST_BE_CREATED]:
			...
			archive_process = Process(
				...
				name="FedBlockSimulator - start_archive process - pow",
				daemon=True
			)
	...
	for index, node in enumerate(run_time_only_params[cm.RunTimeOnlyParams.NODES]):
		try:
			...
			if preloaded_config[cm.ConfigParams.CONSENSUS_PARAMS][AdditionalPowSpecificConfigParams.LIST_OF_ACTIVE_TRAINERS_IN_THE_FIRST_ROUND] is not None:
				if node[cm.RunTimeNodeParams.NODE_ID] in preloaded_config[cm.ConfigParams.CONSENSUS_PARAMS][AdditionalPowSpecificConfigParams.LIST_OF_ACTIVE_TRAINERS_IN_THE_FIRST_ROUND]:
			...
			node_computing_power_factor = None
			for nodes_computing_power_elem in preloaded_config[cm.ConfigParams.CONSENSUS_PARAMS][AdditionalPowSpecificConfigParams.NODES_COMPUTING_POWER]:
				if node[cm.RunTimeNodeParams.NODE_ID] in nodes_computing_power_elem[NodesComputingPowerConfigElemFields.NODE_IDS]:
					node_computing_power_factor = float(nodes_computing_power_elem[NodesComputingPowerConfigElemFields.COMPUTING_POWER_FACTOR])
					break
			
			if node_computing_power_factor is None:
				raise ValueError(f"Node {node[cm.RunTimeNodeParams.NODE_ID]} is not in any nodes computing power element")

			if not is_attacker:
				nodes_configs_in_each_process[process_index][node[cm.RunTimeNodeParams.NODE_ID]] = {
					PowBasedNodeSpecificConfigParams.NODE_ID: node[cm.RunTimeNodeParams.NODE_ID],
					PowBasedNodeSpecificConfigParams.HOST: node[cm.RunTimeNodeParams.HOST],
					PowBasedNodeSpecificConfigParams.PORT: node[cm.RunTimeNodeParams.PORT],
					PowBasedNodeSpecificConfigParams.NUM_OF_QUANTA_TO_USE: node_num_of_quanta_to_use,
					PowBasedNodeSpecificConfigParams.NUM_OF_IID_QUANTA_TO_USE: node_num_of_iid_quanta_to_use,
					PowBasedNodeSpecificConfigParams.ACTIVE_TRAINER_IN_FIRST_ROUND: node_active_in_first_round,
					PowBasedNodeSpecificConfigParams.ALLOWED_TO_PRODUCE_DEBUG_LOG_MESSAGES: node[cm.RunTimeNodeParams.ALLOWED_TO_PRODUCE_DEBUG_LOG_MESSAGES],
					PowBasedNodeSpecificConfigParams.PAUSE_RESUME_EVENT: pause_and_resume_event,
					PowBasedNodeSpecificConfigParams.COMPUTING_POWER_FACTOR: node_computing_power_factor,
					PowBasedNodeSpecificConfigParams.MALICIOUS: is_attacker
				}
			else:
				nodes_configs_in_each_process[process_index][node[cm.RunTimeNodeParams.NODE_ID]] = {
					PowBasedAttackerSpecificConfigParams.NODE_ID: node[cm.RunTimeNodeParams.NODE_ID],
					PowBasedAttackerSpecificConfigParams.HOST: node[cm.RunTimeNodeParams.HOST],
					PowBasedAttackerSpecificConfigParams.PORT: node[cm.RunTimeNodeParams.PORT],
					PowBasedAttackerSpecificConfigParams.NUM_OF_QUANTA_TO_USE: node_num_of_quanta_to_use,
					PowBasedAttackerSpecificConfigParams.NUM_OF_IID_QUANTA_TO_USE: node_num_of_iid_quanta_to_use,
					PowBasedAttackerSpecificConfigParams.ACTIVE_TRAINER_IN_FIRST_ROUND: node_active_in_first_round,
					PowBasedAttackerSpecificConfigParams.ALLOWED_TO_PRODUCE_DEBUG_LOG_MESSAGES: node[cm.RunTimeNodeParams.ALLOWED_TO_PRODUCE_DEBUG_LOG_MESSAGES],
					PowBasedAttackerSpecificConfigParams.PAUSE_RESUME_EVENT: pause_and_resume_event,
					PowBasedAttackerSpecificConfigParams.COMPUTING_POWER_FACTOR: node_computing_power_factor,
					PowBasedAttackerSpecificConfigParams.MALICIOUS: is_attacker,
					PowBasedAttackerSpecificConfigParams.TYPE: behaviour,
					PowBasedAttackerSpecificConfigParams.SELECTED_CLASSES: selected_classes,
					PowBasedAttackerSpecificConfigParams.TARGET_CLASSES: target_classes,
					PowBasedAttackerSpecificConfigParams.TARGET_CLASS: target_class,
					PowBasedAttackerSpecificConfigParams.SQUARE_SIZE: square_size,
					PowBasedAttackerSpecificConfigParams.SIGMA: sigma,
					PowBasedAttackerSpecificConfigParams.NUM_OF_SAMPLES: num_of_samples,
					PowBasedAttackerSpecificConfigParams.STARTING_ROUND_FOR_MALICIOUS_BEHAVIOUR: starting_round_for_malicious_behaviour
				}
			...
			nodes_processes.append(
				(
					Process(
						...
						name=f"FedBlockSimulator - start_nodes process {process_id} - pow"
