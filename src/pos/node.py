import gc, time, hashlib, datetime, sys
...
from ..shared.enums import common as cm, peer_generic as pg, archive_generic as ag, archive_messages as shared_am, node_generic as ng, node_messages as nm
from ..shared.block import BlockType

from .enums import archive_messages as am
from .block import ModelBlock, ModelBlockInvolvedTrainerFields, GenesisBlock, ModelBlockNodeStakesFields

class PosBasedNode(GenericNode):
	...
	def __init__(self, logger: diagnostic.Diagnostic, node_id: int, host: str, port: int, dataset_quanta_paths: list, archive_host: str, archive_port: int, lazy_loading: bool, test_set_path: str, val_set_path: str, malicious_training_semaphore: BoundedSemaphoreClass, honest_training_semaphore: BoundedSemaphoreClass, validation_semaphore: BoundedSemaphoreClass, fit_epochs: int = 3, batch_size: int = 32, already_available_peers: dict = {}, allowed_to_write_redudant_log_messages: bool = False, test_set_validation_of_global_model_after_each_round: bool = False, genesis_block = None, is_active_trainer_in_first_round: (bool | None) = None, store_weights_directly_in_archive_tmp_dir: bool = False, archive_tmp_dir: (str | None) = None, prepare_node_for_first_round: bool = True):
		...
		if type(prepare_node_for_first_round) != bool:
			raise TypeError("PosBasedNode constructor")
		...
		#
		# Pos-specific initializations
		#

		self._node_stakes = {}
		self._amount_of_stake_to_add = 0
		...
		self._num_of_validators = genesis_block.get_num_of_validators()
		self._amount_of_stake_to_add = genesis_block.get_amount_of_stake_to_add()
		self._validators_list = genesis_block.get_list_of_first_validators()
		self._positive_threshold_to_pass_validation = genesis_block.get_threshold_to_pass_validation()
		...
		for elem in genesis_block.get_node_stakes():
			self._node_stakes[elem[ModelBlockNodeStakesFields.NODE_ID]] = elem[ModelBlockNodeStakesFields.STAKE]
		...
		if self._is_validator is True and self._is_active_trainer_in_first_round is True:
			raise ValueError("Node constructor. Node is both a validator and an active trainer in the first round")
		elif self._node_stakes.keys() != already_available_peers.keys():
			raise ValueError(f"Node constructor. Node stakes are not defined for all the available peers. Node stakes: {self._node_stakes.keys()}. Available peers: {already_available_peers.keys()}")
		elif self._peer_id not in self._node_stakes:
			raise ValueError(f"Node constructor. Node stake is not defined for the current peer. Peer id: {self._peer_id}. Node stakes: {self._node_stakes.keys()}")
		...
		if recent_aggregated_model_block_found is False:
			if self.is_validator():
				if self._allowed_to_write_redudant_log_messages:
					self._logger.record(msg = f"Node is a validator", logLevel = diagnostic.DEBUG, identifier= self._str_identifier)

			elif self._is_active_trainer_in_first_round is True or (self._is_active_trainer_in_first_round is None and self._peer_id in self._define_nodes_active_in_the_next_round(SEED)):
	...
	def run(self):
		...
		if self.is_validator() is False and self.aggregation_round() == 1:
	...
	def _define_nodes_active_in_the_next_round(self, seed: int = SEED, base_list_of_nodes: (list | None) = None):
		...
		with self._validators_list_lock:
			if base_list_of_nodes is not None:
					for peer_id in base_list_of_nodes:
						if peer_id not in self._validators_list:
							list_of_available_trainers.append(peer_id)
						
			else:		
				with self._peers_lock:
					for peer_id in self._peers:
						if peer_id not in self._validators_list and self._peers[peer_id][pg.PeersListElement.STATUS]:
							list_of_available_trainers.append(peer_id)

				if len(list_of_available_trainers) != self.get_number_of_peers_alive() - self._num_of_validators:
					raise Exception(f"Invalid number of available nodes. Available nodes: {len(list_of_available_trainers)}. Expected number of available nodes: {self.get_number_of_peers_alive() - self._num_of_validators}")
	...
	def _elect_new_validators(self, latest_block_hash: str, available_nodes_for_next_round: list):
		...
		try:
			...
			elif len(available_nodes_for_next_round) == 0:
				raise ValueError("Node _elect_new_validators method")

			lottery_tickets = dict()
			count_of_sold_tickets = 0
			count_of_winning_tickets = 0
			list_of_hash_bytes = list(bytes.fromhex(latest_block_hash))				# Convert the hash in a list of bytes considering each couple of characters as a byte
			...
			with self._validators_list_lock:
				...
				for node_id in available_nodes_for_next_round:
					if node_id not in self._node_stakes:
						raise Exception(f"Node id not in the stakes table. Node id: {node_id}")
					
					lottery_tickets[node_id] = (count_of_sold_tickets, count_of_sold_tickets + self._node_stakes[node_id])
					count_of_sold_tickets += self._node_stakes[node_id]
				...
				# This algorithm is copied from the one used in Biscotti --> https://github.dev/DistributedML/Biscotti/tree/master/DistSys --> getVRFRoles function
				while len(self._validators_list) < self._num_of_validators:
					
					if count_of_winning_tickets >= len(list_of_hash_bytes):
						count_of_winning_tickets = 0
						
						# Compute the hash of the hash
						list_of_hash_bytes = list(hashlib.sha256(bytes(list_of_hash_bytes)).digest())

					winner_idx = (list_of_hash_bytes[count_of_winning_tickets] * 256 + list_of_hash_bytes[count_of_winning_tickets + 1]) % count_of_sold_tickets

					for node_id, tickets in lottery_tickets.items():
						if tickets[0] <= winner_idx < tickets[1]:
							if node_id not in self._validators_list:
								self._validators_list.append(node_id)
								if self._allowed_to_write_redudant_log_messages:
									self._logger.record(msg = f"Extracted ticket: {winner_idx}. Winning node: {node_id}", logLevel= diagnostic.DEBUG, identifier= self._str_identifier)
							
							elif self._allowed_to_write_redudant_log_messages:
								self._logger.record(msg = f"Extracted ticket: {winner_idx}. Duplicated winning node: {node_id}", logLevel= diagnostic.DEBUG, identifier= self._str_identifier)

							break
					
					count_of_winning_tickets += 1

				if self._allowed_to_write_redudant_log_messages:
					self._logger.record(msg = f"New validators elected. Validators ids: {self._validators_list}. Round: {self.aggregation_round()}", logLevel= diagnostic.DEBUG, identifier= self._str_identifier)

				if self._peer_id in self._validators_list:
					self._is_validator = True

		except Exception as e:
			self._logger.record(msg = f"Error while electing the new validators", exc = e, logLevel = diagnostic.ERROR, identifier= self._str_identifier)
			raise e
		...
		try:
			...
			if stop_training is False:
				...
				if available_nodes_for_next_round is None or updater_ids_update_names_aggregated_validation_scores is None:
					...
					# Update the stakes (I don't use clear method because otherwise every reference to the dictionary will be cleared)
					self._node_stakes = dict()

					for elem in last_aggregated_model_block.get_node_stakes():
						self._node_stakes[elem[ModelBlockNodeStakesFields.NODE_ID]] = elem[ModelBlockNodeStakesFields.STAKE]
				...
				# If the node is not validator, we need to check if the node is an active trainer in the next round, in that case we need to load the newest model weights and start the training
				if not self.is_validator():

					if self._allowed_to_write_redudant_log_messages:
						self._logger.record(msg = f"Node is not part of the next list of validators. Round: {self.aggregation_round()}", logLevel = diagnostic.DEBUG, identifier= self._str_identifier)
					...
					self._logger.record(msg = f"Node is part of the next list of validators. Round: {self.aggregation_round()}", logLevel = diagnostic.INFO, identifier= self._str_identifier)
	...
	def _create_and_upload_aggregated_model_block(self, update_names_and_aggregated_scores: dict):
		...
		try:
			...
			# Define the list of available nodes for the next round and update the stakes
			...
			with self._peers_lock:
				for peer_id in self._peers:
					...
					if peer_id not in self._node_stakes:
						self._node_stakes[peer_id] = 0
				
			for elem in updater_ids_update_names_aggregated_validation_scores:
				self._node_stakes[elem[ModelBlockInvolvedTrainerFields.NODE_ID]] += self._amount_of_stake_to_add
			
			with self._validators_list_lock:
				for validator_id in self._validators_list:
					self._node_stakes[validator_id] += self._amount_of_stake_to_add
			
			# Convert the node stakes in the right format to avoid the problem of json.dumps with integer keys
			node_ids_node_stakes = list()

			for node_id, node_stake in self._node_stakes.items():
				node_ids_node_stakes.append({ModelBlockNodeStakesFields.NODE_ID: node_id, ModelBlockNodeStakesFields.STAKE: node_stake})

			request = shared_am.GenericArchiveRequest.builder(shared_am.ArchiveRequestTypes.UPLOAD_BLOCKCHAIN_AGGREGATED_MODEL, am.UploadBlockchainAggregatedModelRequestBody.builder(self._peer_id, model_name, self.aggregation_round(), time.time(), updater_ids_update_names_aggregated_validation_scores, self._validators_list, list_of_available_nodes_for_next_round, node_ids_node_stakes), self.aggregation_round())
			...
			self._logger.record(msg = f"New aggregated model block uploaded. Node stakes: {self._node_stakes}", logLevel = diagnostic.DEBUG, identifier= self._str_identifier, skipLengthTruncation= True)
			...
			self._send_message(nm.BroadcastNewModelBlockAlert.builder(agg_block_hash, self._peer_id, self.aggregation_round()))
