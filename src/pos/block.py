...
class ModelBlockNodeStakesFields(AbstractEnum):
    NODE_ID = "node_id"
    STAKE = "stake"
  
class ModelBlock(Block):
    def __init__(self, previous_hash: str, timestamp: float, global_model_name: str, aggregation_round: int, involved_trainers: list, list_of_current_validators: list, available_nodes: list, node_stakes: list, block_hash: (str | None) = None):

        if type(previous_hash) != str or type(timestamp) not in [float, int] or type(global_model_name) != str or type(aggregation_round) != int or type(involved_trainers) != list or type(list_of_current_validators) != list or type(available_nodes) != list or type(node_stakes) != list or (block_hash is not None and type(block_hash) != str):
        ...
        elif any(key not in elem for key in ModelBlockNodeStakesFields.list() for elem in node_stakes):
            raise ValueError("ModelBlock. Node stakes")
        ...
        for elem in node_stakes:
            if type(elem[ModelBlockNodeStakesFields.NODE_ID]) != int or type(elem[ModelBlockNodeStakesFields.STAKE]) not in [int, float]:
                raise TypeError(f"ModelBlock. Node stakes elem: {elem}")
    ...
    def from_json(cls, json_string: str):
        ...
        if any([elem not in data for elem in ["previous_hash", "timestamp", "global_model_name", "aggregation_round", "involved_trainers", "list_of_current_validators", "available_nodes", "node_stakes", "block_hash"]]):
            raise ValueError("ModelBlock.from_json")

        return cls(data["previous_hash"], data["timestamp"], data["global_model_name"], data["aggregation_round"], data["involved_trainers"], data["list_of_current_validators"], data["available_nodes"], data["node_stakes"], data["block_hash"])

class GenesisBlock(Block):

    def __init__(self, model_architecture: dict, model_starting_weights: list, model_starting_optimizer_state: dict | None, max_num_of_aggregation_rounds: int, num_of_validators: int, node_stakes: list, amount_of_stake_to_add: float, perc_of_nodes_active_in_a_round: float, threshold_to_pass_validation: int, list_of_first_validators: list, validation_algorithm_params: dict, aggregation_algorithm_params: dict, block_hash: (str | None) = None):
        
        if type(model_architecture) != dict or type(model_starting_weights) != list or (model_starting_optimizer_state is not None and type(model_starting_optimizer_state) != dict) or type(max_num_of_aggregation_rounds) != int or type(num_of_validators) != int or type(node_stakes) != list or type(amount_of_stake_to_add) not in [int, float] or type(perc_of_nodes_active_in_a_round) not in [int, float] or type(threshold_to_pass_validation) != int or type(list_of_first_validators) != list or type(validation_algorithm_params) != dict or type(aggregation_algorithm_params) != dict or (block_hash is not None and type(block_hash) != str):
        ...
        self._node_stakes = node_stakes
        self._amount_of_stake_to_add = amount_of_stake_to_add
    ...
    def get_amount_of_stake_to_add(self):
        return self._amount_of_stake_to_add
    ...
    def to_json(self):
        return json.dumps({
            ...
            "amount_of_stake_to_add": self._amount_of_stake_to_add,
            "node_stakes": self._node_stakes,
    ...
    def from_json(cls, json_string: str):
        ...
        if any([elem not in data for elem in ["model_architecture", "model_starting_weights", "model_starting_optimizer_state", "max_num_of_aggregation_rounds", "number_of_validators", "threshold_to_pass_validation", "list_of_first_validators", "perc_of_nodes_active_in_a_round", "validation_algorithm_params", "aggregation_algorithm_params", "block_hash", "node_stakes", "amount_of_stake_to_add"]]):
            raise ValueError("GenesisBlock.from_json")
        
        return cls(data["model_architecture"], data["model_starting_weights"], data["model_starting_optimizer_state"], data["max_num_of_aggregation_rounds"], data["number_of_validators"], data["node_stakes"], data["amount_of_stake_to_add"], data["perc_of_nodes_active_in_a_round"], data["threshold_to_pass_validation"], data["list_of_first_validators"], data["validation_algorithm_params"], data["aggregation_algorithm_params"], data["block_hash"])
