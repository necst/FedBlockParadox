...
from ..shared.enums.peer_generic import PeersListElement
from ..shared.enums import validation_algos as va, common as cm
...
from .enums.common import AdditionalPosSpecificConfigParams, AdditionalPosSpecificConfigKnownInitialNodesStakesFields
from .constants import DEFAULT_POS_SPECIFIC_CONFIG
from .block import GenesisBlock, ModelBlockNodeStakesFields
from .archive import PosBasedArchive
from .node import PosBasedNode
from .attacker_nodes import PosBasedAdditiveNoiseByzantineNode, PosBasedRandomNoiseByzantineNode, PosBasedRandomLabelByzantineNode, PosBasedLabelFlippingNode, PosBasedTargetedPoisoningNode
...
def validate_pos_consensus_specific_config(stored_config: dict, run_time_only_params: dict) -> None:
	'''
	Define the pos specific running configuration from the stored configuration.
	'''

	try:
		if type(stored_config) != dict or type(run_time_only_params) != dict:
			raise TypeError("validate_pos_consensus_specific_config")
		elif cm.ConfigParams.CONSENSUS_PARAMS not in stored_config:
			raise KeyError("validate_pos_consensus_specific_config. Consensus params not in stored configuration")
		elif type(stored_config[cm.ConfigParams.CONSENSUS_PARAMS]) != dict:
			raise TypeError("validate_pos_consensus_specific_config. Consensus params is not a dictionary")
		elif cm.ConfigGenericConsensusAlgorithmParams.TYPE not in stored_config[cm.ConfigParams.CONSENSUS_PARAMS] or stored_config[cm.ConfigParams.CONSENSUS_PARAMS][cm.ConfigGenericConsensusAlgorithmParams.TYPE] != cm.ConsensusAlgorithmType.POS:
			raise ValueError("validate_pos_consensus_specific_config. Consensus algorithm type is not pos specific")
		elif any(key not in run_time_only_params for key in cm.RunTimeOnlyParams.list()):
			raise KeyError("validate_pos_consensus_specific_config. Run time only params missing key")
		...
		for key in AdditionalPosSpecificConfigParams.list():
			if key not in consensus_specific_config:
				raise KeyError(f"validate_pos_consensus_specific_config. {key} not in stored configuration")
		...
		if is_support_machine_config is True:
			if consensus_specific_config[AdditionalPosSpecificConfigParams.LIST_OF_ACTIVE_TRAINERS_IN_THE_FIRST_ROUND] is None:
				raise ValueError("validate_pos_consensus_specific_config. List of active trainers in the first round is None but it is required for the support machine")

		if consensus_specific_config[AdditionalPosSpecificConfigParams.LIST_OF_ACTIVE_TRAINERS_IN_THE_FIRST_ROUND] is not None:
			if type(consensus_specific_config[AdditionalPosSpecificConfigParams.LIST_OF_ACTIVE_TRAINERS_IN_THE_FIRST_ROUND]) != list:
				raise TypeError("validate_pos_consensus_specific_config. List of active trainers in the first round is not a list")
			elif any(type(elem) != int or elem < 0 for elem in consensus_specific_config[AdditionalPosSpecificConfigParams.LIST_OF_ACTIVE_TRAINERS_IN_THE_FIRST_ROUND]):
				raise TypeError("validate_pos_consensus_specific_config. List of active trainers in the first round contains an element that is not an integer or is less than zero")
			
			for node_id in consensus_specific_config[AdditionalPosSpecificConfigParams.LIST_OF_ACTIVE_TRAINERS_IN_THE_FIRST_ROUND]:
				if any(node_id == node[cm.NodeSpecificConfigParams.NODE_ID] for node in run_time_only_params[cm.RunTimeOnlyParams.NODES]) is False:
					raise ValueError("validate_pos_consensus_specific_config. List of active trainers in the first round contains a node id that is not part of the simulation")

		if is_support_machine_config is False:
			# Number of validators
			if consensus_specific_config[AdditionalPosSpecificConfigParams.NUM_OF_VALIDATORS] is None:
				raise ValueError("validate_pos_consensus_specific_config. Number of validators is None")
			elif type(consensus_specific_config[AdditionalPosSpecificConfigParams.NUM_OF_VALIDATORS]) != int:
				raise TypeError("validate_pos_consensus_specific_config. Number of validators is not an integer")
			elif consensus_specific_config[AdditionalPosSpecificConfigParams.NUM_OF_VALIDATORS] < 0:
				raise ValueError("validate_pos_consensus_specific_config. Number of validators is less than zero")
			elif consensus_specific_config[AdditionalPosSpecificConfigParams.NUM_OF_VALIDATORS] > len(run_time_only_params[cm.RunTimeOnlyParams.NODES]):
				raise ValueError("validate_pos_consensus_specific_config. Number of validators is greater than the number of nodes in the simulation")

			# List of node ids of first validators
			if consensus_specific_config[AdditionalPosSpecificConfigParams.LIST_OF_NODE_IDS_OF_FIRST_VALIDATORS] is not None:
				if type(consensus_specific_config[AdditionalPosSpecificConfigParams.LIST_OF_NODE_IDS_OF_FIRST_VALIDATORS]) != list:
					raise TypeError("validate_pos_consensus_specific_config. List of node ids of first validators is not a list")
				elif any(type(elem) != int or elem < 0 for elem in consensus_specific_config[AdditionalPosSpecificConfigParams.LIST_OF_NODE_IDS_OF_FIRST_VALIDATORS]):
					raise TypeError("validate_pos_consensus_specific_config. List of node ids of first validators contains an element that is not an integer or is less than zero")
				elif len(consensus_specific_config[AdditionalPosSpecificConfigParams.LIST_OF_NODE_IDS_OF_FIRST_VALIDATORS]) != consensus_specific_config[AdditionalPosSpecificConfigParams.NUM_OF_VALIDATORS]:
					raise ValueError("validate_pos_consensus_specific_config. Number of node ids of first validators is different from the number of validators")
				
				for node_id in consensus_specific_config[AdditionalPosSpecificConfigParams.LIST_OF_NODE_IDS_OF_FIRST_VALIDATORS]:
					if consensus_specific_config[AdditionalPosSpecificConfigParams.LIST_OF_ACTIVE_TRAINERS_IN_THE_FIRST_ROUND] is not None:
						if node_id in consensus_specific_config[AdditionalPosSpecificConfigParams.LIST_OF_ACTIVE_TRAINERS_IN_THE_FIRST_ROUND]:
							raise ValueError("validate_pos_consensus_specific_config. Node id of first validators is also part of the list of active trainers in the first round")
						
					if any(node_id == node[cm.NodeSpecificConfigParams.NODE_ID] for node in run_time_only_params[cm.RunTimeOnlyParams.NODES]) is False:
						raise ValueError("validate_pos_consensus_specific_config. List of node ids of first validators contains a node id that is not part of the simulation")
			
			# Threshold to pass validation
			if consensus_specific_config[AdditionalPosSpecificConfigParams.PERC_THRESHOLD_TO_PASS_VALIDATION] is None:
				raise ValueError("validate_pos_consensus_specific_config. Threshold to pass validation is None")
			elif type(consensus_specific_config[AdditionalPosSpecificConfigParams.PERC_THRESHOLD_TO_PASS_VALIDATION]) not in [float, int]:
				raise TypeError("validate_pos_consensus_specific_config. Threshold to pass validation is not a float or int")
			elif consensus_specific_config[AdditionalPosSpecificConfigParams.PERC_THRESHOLD_TO_PASS_VALIDATION] < 0.0 or consensus_specific_config[AdditionalPosSpecificConfigParams.PERC_THRESHOLD_TO_PASS_VALIDATION] > 1.0:
				raise ValueError("validate_pos_consensus_specific_config. Threshold to pass validation is not between 0 and 1")
		
			# Percentage of trainers active in each round
			if consensus_specific_config[AdditionalPosSpecificConfigParams.PERC_OF_TRAINERS_ACTIVE_IN_EACH_ROUND] is None:
				raise ValueError("validate_pos_consensus_specific_config. Percentage of trainers active in each round is None")
			elif type(consensus_specific_config[AdditionalPosSpecificConfigParams.PERC_OF_TRAINERS_ACTIVE_IN_EACH_ROUND]) not in [float, int]:
				raise TypeError("validate_pos_consensus_specific_config. Percentage of trainers active in each round is not a float or int")
			elif consensus_specific_config[AdditionalPosSpecificConfigParams.PERC_OF_TRAINERS_ACTIVE_IN_EACH_ROUND] < 0.0 or consensus_specific_config[AdditionalPosSpecificConfigParams.PERC_OF_TRAINERS_ACTIVE_IN_EACH_ROUND] > 1.0:
				raise ValueError("validate_pos_consensus_specific_config. Percentage of trainers active in each round is not between 0 and 1")
			...
			num_of_trainers_in_each_round = round(consensus_specific_config[AdditionalPosSpecificConfigParams.PERC_OF_TRAINERS_ACTIVE_IN_EACH_ROUND] * (len(run_time_only_params[cm.RunTimeOnlyParams.NODES]) - consensus_specific_config[AdditionalPosSpecificConfigParams.NUM_OF_VALIDATORS]))

			# Default initial node stake
			if consensus_specific_config[AdditionalPosSpecificConfigParams.DEFAULT_INITIAL_NODE_STAKE] is None:
				raise ValueError("validate_pos_consensus_specific_config. Default initial node stake is None")
			elif type(consensus_specific_config[AdditionalPosSpecificConfigParams.DEFAULT_INITIAL_NODE_STAKE]) != int:
				raise TypeError("validate_pos_consensus_specific_config. Default initial node stake is not an integer")
			elif consensus_specific_config[AdditionalPosSpecificConfigParams.DEFAULT_INITIAL_NODE_STAKE] < 0:
				raise ValueError("validate_pos_consensus_specific_config. Default initial node stake is less than zero")

			# Known initial nodes stakes
			if consensus_specific_config[AdditionalPosSpecificConfigParams.KNOWN_INITIAL_NODES_STAKES] is None:
				raise ValueError("validate_pos_consensus_specific_config. Known initial nodes stakes is None")
			elif type(consensus_specific_config[AdditionalPosSpecificConfigParams.KNOWN_INITIAL_NODES_STAKES]) != list:
				raise TypeError("validate_pos_consensus_specific_config. Known initial nodes stakes is not a list")
			
			for elem in consensus_specific_config[AdditionalPosSpecificConfigParams.KNOWN_INITIAL_NODES_STAKES]:
				if type(elem) != dict:
					raise TypeError("validate_pos_consensus_specific_config. Known initial nodes stakes contains an element that is not a dictionary")
				elif any(key not in elem for key in AdditionalPosSpecificConfigKnownInitialNodesStakesFields.list()):
					raise KeyError("validate_pos_consensus_specific_config. Known initial nodes stakes contains an element that is missing a key")
				
				if type(elem[AdditionalPosSpecificConfigKnownInitialNodesStakesFields.NODE_ID]) != int:
					raise TypeError("validate_pos_consensus_specific_config. Known initial nodes stakes contains an element with a node id that is not an integer")
				elif any(elem[AdditionalPosSpecificConfigKnownInitialNodesStakesFields.NODE_ID] == node[cm.NodeSpecificConfigParams.NODE_ID] for node in run_time_only_params[cm.RunTimeOnlyParams.NODES]) is False:
					raise ValueError("validate_pos_consensus_specific_config. Known initial nodes stakes contains a node id that is not part of the simulation")
				
				if type(elem[AdditionalPosSpecificConfigKnownInitialNodesStakesFields.STAKE]) != int:
					raise TypeError("validate_pos_consensus_specific_config. Known initial nodes stakes contains an element with a stake that is not an integer")
				elif elem[AdditionalPosSpecificConfigKnownInitialNodesStakesFields.STAKE] < 0:
					raise ValueError("validate_pos_consensus_specific_config. Known initial nodes stakes contains an element with a stake that is less than zero")

			# Amount of stake to add
			if consensus_specific_config[AdditionalPosSpecificConfigParams.AMOUNT_OF_STAKE_TO_ADD] is None:
				raise ValueError("validate_pos_consensus_specific_config. Amount of stake to add is None")
			elif type(consensus_specific_config[AdditionalPosSpecificConfigParams.AMOUNT_OF_STAKE_TO_ADD]) != int:
				raise TypeError("validate_pos_consensus_specific_config. Amount of stake to add is not an integer")
			elif consensus_specific_config[AdditionalPosSpecificConfigParams.AMOUNT_OF_STAKE_TO_ADD] < 0:
				raise ValueError("validate_pos_consensus_specific_config. Amount of stake to add is less than zero")	
			...
			if stored_config[cm.ConfigParams.VALIDATION_PARAMS][cm.ConfigGenericConsensusAlgorithmParams.TYPE] == cm.ValidationAlgorithmType.LOCAL_DATASET_VALIDATION:
				if num_of_trainers_in_each_round < stored_config[cm.ConfigParams.VALIDATION_PARAMS][va.LocalDatasetUsedForValidationParams.MINIMUM_NUM_OF_UPDATES_BETWEEN_AGGREGATIONS]:
					raise ValueError("validate_pos_consensus_specific_config. Number of trainers in each round is less than the minimum number of updates between aggregations. Consequently, it is not possible to complete any round")
			...
			# Krum validation
				...
				if num_of_trainers_in_each_round < stored_config[cm.ConfigParams.VALIDATION_PARAMS][va.KrumValidationParams.MINIMUM_NUM_OF_UPDATES_NEEDED_TO_START_VALIDATION]:
					raise ValueError("validate_pos_consensus_specific_config. Number of trainers in each round is less than the minimum number of updates needed to start the validation. Consequently, it is not possible to complete any round")
			...
			# Pass gradients validation
				...
				if num_of_trainers_in_each_round < stored_config[cm.ConfigParams.VALIDATION_PARAMS][va.PassGradientsValidationParams.MINIMUM_NUM_OF_UPDATES_NEEDED_TO_START_VALIDATION]:
					raise ValueError("validate_pos_consensus_specific_config. Number of trainers in each round is less than the number of updates between aggregations. Consequently, it is not possible to complete any round")
			
			else:
				raise ValueError("validate_pos_consensus_specific_config. Invalid validation algorithm")

	except Exception as e:
		raise Exception(f"validate_pos_consensus_specific_config. Exception: {e}") from e
		...
		for node_id in nodes_specific_params:
			...
			if not nodes_specific_params[node_id][cm.NodeSpecificConfigParams.MALICIOUS]:
				...
				if nodes_specific_params[node_id][cm.AttackerSpecificConfigParams.TYPE] == cm.MaliciousNodeBehaviourType.LABEL_FLIPPING:
					...
					nodes_threads.append(
						PosBasedRandomLabelByzantineNode(
							logger,
							nodes_specific_params[node_id][cm.AttackerSpecificConfigParams.NODE_ID],
							nodes_specific_params[node_id][cm.AttackerSpecificConfigParams.HOST],
							nodes_specific_params[node_id][cm.AttackerSpecificConfigParams.PORT],
					...
					nodes_threads.append(
						PosBasedAdditiveNoiseByzantineNode(
							logger,
							nodes_specific_params[node_id][cm.AttackerSpecificConfigParams.NODE_ID],
							nodes_specific_params[node_id][cm.AttackerSpecificConfigParams.HOST],
							nodes_specific_params[node_id][cm.AttackerSpecificConfigParams.PORT],
					...
					nodes_threads.append(
						PosBasedRandomNoiseByzantineNode(
							logger,
							nodes_specific_params[node_id][cm.AttackerSpecificConfigParams.NODE_ID],
							nodes_specific_params[node_id][cm.AttackerSpecificConfigParams.HOST],
							nodes_specific_params[node_id][cm.AttackerSpecificConfigParams.PORT],
...
def start_pos_simulation_main(logger: diagnostic.Diagnostic, quanta_paths: list, testset_path: str, valset_path: str, config_file_path: str, preloaded_config: dict, run_time_only_params: dict) -> None:
	'''Start the pos-based simulation'''
	...
	if isinstance(logger, diagnostic.Diagnostic) is False or type(quanta_paths) != list or type(testset_path) != str or type(valset_path) != str or type(config_file_path) != str or type(preloaded_config) != dict or type(run_time_only_params) != dict:
		raise TypeError("start_pos_simulation_main")
		...
		raise FileNotFoundError("start_pos_simulation_main")
	elif any(key not in preloaded_config for key in cm.ConfigParams.list()):
		raise KeyError("start_pos_simulation_main. cm.ConfigParams missing key")
	elif any(key not in run_time_only_params for key in cm.RunTimeOnlyParams.list()):
		raise KeyError("start_pos_simulation_main. Run time only params missing key")

	is_support_simulation = not preloaded_config[cm.ConfigParams.IS_MAIN_SIMULATION]
	
	# Load the configuration file
	preloaded_config[cm.ConfigParams.CONSENSUS_PARAMS] = {**DEFAULT_POS_SPECIFIC_CONFIG, **preloaded_config[cm.ConfigParams.CONSENSUS_PARAMS]}

	validate_pos_consensus_specific_config(preloaded_config, run_time_only_params)
	...
	if is_support_simulation is False:
		...
		# First validators
		if preloaded_config[cm.ConfigParams.CONSENSUS_PARAMS][AdditionalPosSpecificConfigParams.LIST_OF_NODE_IDS_OF_FIRST_VALIDATORS] is not None:
			nodes_ids_of_first_validators = preloaded_config[cm.ConfigParams.CONSENSUS_PARAMS][AdditionalPosSpecificConfigParams.LIST_OF_NODE_IDS_OF_FIRST_VALIDATORS]
		
		else:
			nodes_ids_ready_for_being_validators_list = list()
			nodes_ids_not_available = list()

			if preloaded_config[cm.ConfigParams.CONSENSUS_PARAMS][AdditionalPosSpecificConfigParams.LIST_OF_ACTIVE_TRAINERS_IN_THE_FIRST_ROUND] is not None:
				nodes_ids_not_available = preloaded_config[cm.ConfigParams.CONSENSUS_PARAMS][AdditionalPosSpecificConfigParams.LIST_OF_ACTIVE_TRAINERS_IN_THE_FIRST_ROUND]
			...
			if len(nodes_ids_ready_for_being_validators_list) < preloaded_config[cm.ConfigParams.CONSENSUS_PARAMS][AdditionalPosSpecificConfigParams.NUM_OF_VALIDATORS]:
				raise ValueError("start_pos_simulation_main. Number of nodes that can be validators is less than the required number of validators")
		
			nodes_ids_of_first_validators = np.random.choice(nodes_ids_ready_for_being_validators_list, preloaded_config[cm.ConfigParams.CONSENSUS_PARAMS][AdditionalPosSpecificConfigParams.NUM_OF_VALIDATORS], replace=False).tolist()
	...
	for elem in preloaded_config[cm.ConfigParams.CONSENSUS_PARAMS][AdditionalPosSpecificConfigParams.KNOWN_INITIAL_NODES_STAKES]:
		known_initial_nodes_stakes[elem[AdditionalPosSpecificConfigKnownInitialNodesStakesFields.NODE_ID]] = elem[AdditionalPosSpecificConfigKnownInitialNodesStakesFields.STAKE]

	if is_support_simulation is False:
		current_timestamp = time.time()

		for node in run_time_only_params[cm.RunTimeOnlyParams.NODES]:
			initial_peers_dict[node[cm.RunTimeNodeParams.NODE_ID]] = {PeersListElement.HOST: node[cm.RunTimeNodeParams.HOST], PeersListElement.PORT: node[cm.RunTimeNodeParams.PORT], PeersListElement.STATUS: True, PeersListElement.LAST_UPDATE_TIMESTAMP: current_timestamp}
			
			if node[cm.RunTimeNodeParams.NODE_ID] in known_initial_nodes_stakes:
				node_stakes.append({ModelBlockNodeStakesFields.NODE_ID: node[cm.RunTimeNodeParams.NODE_ID], ModelBlockNodeStakesFields.STAKE: known_initial_nodes_stakes[node[cm.RunTimeNodeParams.NODE_ID]]})
			else:
				node_stakes.append({ModelBlockNodeStakesFields.NODE_ID: node[cm.RunTimeNodeParams.NODE_ID], ModelBlockNodeStakesFields.STAKE: preloaded_config[cm.ConfigParams.CONSENSUS_PARAMS][AdditionalPosSpecificConfigParams.DEFAULT_INITIAL_NODE_STAKE]})
	...
	if is_support_simulation is False:
		threshold_to_pass_validation = round(preloaded_config[cm.ConfigParams.CONSENSUS_PARAMS][AdditionalPosSpecificConfigParams.PERC_THRESHOLD_TO_PASS_VALIDATION] * len(nodes_ids_of_first_validators))
		...
		genesis_block = processes_shared_variables_manager.GenesisBlock(
			...
			# Consensus params
			preloaded_config[cm.ConfigParams.CONSENSUS_PARAMS][AdditionalPosSpecificConfigParams.NUM_OF_VALIDATORS],
			node_stakes,
			preloaded_config[cm.ConfigParams.CONSENSUS_PARAMS][AdditionalPosSpecificConfigParams.AMOUNT_OF_STAKE_TO_ADD],			
			preloaded_config[cm.ConfigParams.CONSENSUS_PARAMS][AdditionalPosSpecificConfigParams.PERC_OF_TRAINERS_ACTIVE_IN_EACH_ROUND],
		...
		if preloaded_config[cm.ConfigParams.ARCHIVE_PARAMS][cm.ConfigArchiveParams.ARCHIVE_MUST_BE_CREATED]:
			...
			archive_process = Process(
				...
				name="FedBlockSimulator - start_archive process - pos",
				daemon=True
			)
			...
			if process_ready_event.wait(240) is False:
				raise TimeoutError("The archive process did not start after 120 seconds of waiting")
	...
	for index, node in enumerate(run_time_only_params[cm.RunTimeOnlyParams.NODES]):
		try:
			...
			if preloaded_config[cm.ConfigParams.CONSENSUS_PARAMS][AdditionalPosSpecificConfigParams.LIST_OF_ACTIVE_TRAINERS_IN_THE_FIRST_ROUND] is not None:
				if node[cm.RunTimeNodeParams.NODE_ID] in preloaded_config[cm.ConfigParams.CONSENSUS_PARAMS][AdditionalPosSpecificConfigParams.LIST_OF_ACTIVE_TRAINERS_IN_THE_FIRST_ROUND]:
			...
			nodes_processes.append(
				(
					Process(
						...
						name=f"FedBlockSimulator - start_nodes process {process_id} - pos"
	...
	for elem in preloaded_config[cm.ConfigParams.CONSENSUS_PARAMS][AdditionalPosSpecificConfigParams.KNOWN_INITIAL_NODES_STAKES]:
		if any(elem[AdditionalPosSpecificConfigKnownInitialNodesStakesFields.NODE_ID] == node[cm.RunTimeNodeParams.NODE_ID] for node in run_time_only_params[cm.RunTimeOnlyParams.NODES]) is False:
			logger.record(f"Node ID {elem[AdditionalPosSpecificConfigKnownInitialNodesStakesFields.NODE_ID]} inside known_initial_nodes_stakes dict is not a valid node ID", diagnostic.WARNING, _MODULE)
			print(f"Node ID {elem[AdditionalPosSpecificConfigKnownInitialNodesStakesFields.NODE_ID]} inside known_initial_nodes_stakes dict is not a valid node ID")
