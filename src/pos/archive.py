...
class PosBasedArchive(GenericArchive):
	...
	def _handle_upload_blockchain_aggregated_model_request(self, msg: dict):
		...
		node_stakes = msg[am.UploadBlockchainAggregatedModelRequestBody.NODE_STAKES]
		...
		try:		
			with self._current_round_number_lock:
				with self._blockchain_lock:
					...
					model = ModelBlock(previous_hash, timestamp, model_name, round, involved_trainers, list_of_current_validators, available_nodes, node_stakes)
